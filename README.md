...
Business applications that read static or slowly changing data from a Databricks Serverless SQL Warehouse can experience cold-start latency, because the warehouse automatically stops after a short idle period to control cost. When the next query arrives, the warehouse must start before serving it, and end users incur that delay.

This cloud-native optimization pattern places an AWS Lambda function and an Amazon DynamoDB table in front of the Databricks workspace as a serverless read-through cache. On a request, Lambda first checks DynamoDB. If the cached item is fresh, it returns immediately. Otherwise, Lambda reads the data from Databricks, writes the result to DynamoDB with an expiry timestamp, and returns the value. An Amazon EventBridge Scheduler invokes the same Lambda on a configurable cadence to refresh entries before they expire, so production reads hit the cache most of the time. AWS Secrets Manager stores the Databricks credentials, and the Lambda execution role follows least-privilege IAM. The pattern reduces typical reads to a single DynamoDB GetItem, preserves Databricks auto-stop cost savings, and shields callers from transient slowness in Databricks.
